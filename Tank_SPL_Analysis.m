...
f = freq_bands(i:end)                           %Use only frequencies from Schroeder's frequency
w = 2*pi*f;                                       %From linear to angular freq.

T60_r = Paper_T60_r(i:end);
T60_t = Paper_T60_t(i:end);
...
xlim([630 5000])
xticklabels({'500','630','800','1k','1.25k','1.6k','2k','2.5k','3.15k','4k','5k'})
yticks([115, 120, 125, mean_SPL(1), 130, 135, 140]);
yticklabels({'115','120', '125', num2str(mean_SPL(1),'\\bf%0.0f'), '130', '135', '140'})
hold on;
semilogx(f,mean_SPL,'b--','LineWidth',2.5)
legend('SPL', 'Mean per Frequency')
text(2500,125,txt)
