...
%% A partir daqui é SEA!
%Nesse ponto tenho o valor em dB, posso passá-los para pressão se quiser, e
%utilizar como fonte sonora. Ressaltar que nesse caso, por simplicidade,
%está sendo considerado uma fonte fictícia produzindo uma espécie de ruído
%branco e utilizando so valores conforme o datasheet baseado nos valores
%previstos por https://www.flofab.com/HomeSite/Literature/Documents/Support/engineering%20data%20and%20useful%20documents/Typical%20Sound%20Power%20Levels%20Of%20Pumpsets.pdf
...
Sroom = 2*(r1*r2) + 2*(r1*r3) + 2*(r2*r3) - (t1*t3);      % [m²] Soma da área das paredes da sala
Lroom =  4*r1+4*r2+4*r3;                           % [m] 
Stank =  2*(t1*t2)+2*(t2*t3) + 2*(t1*t3); 
Ltank =  4*t1+4*t2+4*t3; 
Atank = t1*t3;                                    % Área da superfície do tank
...
%Lembrar que a fórmula de obtenção de potência em dB é diferente de SPL!
...
%% Comentar
% 
% Foi utilizada a informação disponível para que fosse obtida a pressão média, entretanto existem
% limitações. Explicar por que a frequência de schroeder deve ser considerada como o "limite inferior"
% no qual o método é válido e que o fs do tanque é menor que da sala. Além disso, comentar que foi
% utilizado a partir de 630 Hz pq abaixo disso não há informação do Tr do tanque
% 
% Decorar os gráficos
